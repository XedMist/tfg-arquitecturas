...
from models.mixer.global_attention import GlobalAttentionMixerConfig
from models.module.drop_path import calculate_drop_path_rates


def build_small_global_attention(cfg: DictConfig) -> nn.Module:
    ...
    config = MetaFormerConfig(
        ...
        stages=[
            StageConfig(
                in_dim=48,
                out_dim=96,
                mixer_configs=[GlobalAttentionMixerConfig(num_heads=8, d_model=48)]
                * depths[0],
                block_cfgs=[
            ...
            StageConfig(
                in_dim=96,
                out_dim=192,
                mixer_configs=[GlobalAttentionMixerConfig(num_heads=16, d_model=96)]
                * depths[1],
                block_cfgs=[
                    BlockConfig(use_mlp=True, drop_path=dp_rates[i])
