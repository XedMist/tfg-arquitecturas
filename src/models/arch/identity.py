...
from models.mixer.identity import IdentityMixerConfig
from models.module.drop_path import calculate_drop_path_rates


def build_small_identity(cfg: DictConfig) -> nn.Module:
    ...
    config = MetaFormerConfig(
        ...
        stages=[
            StageConfig(
                in_dim=48,
                out_dim=96,
                mixer_configs=[IdentityMixerConfig(48)] * depths[0],
                block_cfgs=[
            ...
            StageConfig(
                in_dim=96,
                out_dim=192,
                mixer_configs=[IdentityMixerConfig(96)] * depths[1],
                block_cfgs=[
                    BlockConfig(use_mlp=True, drop_path=dp_rates[i])
            ...
            StageConfig(
                in_dim=192,
                out_dim=288,
                mixer_configs=[IdentityMixerConfig(192)] * depths[2],
                block_cfgs=[
                    BlockConfig(use_mlp=True, drop_path=dp_rates[i])
            ...
            StageConfig(
                in_dim=288,
                out_dim=288,
                mixer_configs=[IdentityMixerConfig(288)] * depths[3],
                block_cfgs=[
                    BlockConfig(use_mlp=True, drop_path=dp_rates[i])
