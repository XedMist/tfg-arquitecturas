...
@register_mixer(DeformableAttentionMixerConfig)
class DATDeformableMixer(nn.Module):
    """
    Modulo de atencion deformable
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fixed_pe = config.fixed_pe
        self.no_off = config.no_off
        self.offset_range_factor = config.offset_range_factor
        self.ksize = config.ksize
        self.log_cpb = config.log_cpb
        self.stride = config.stride
        kk = self.ksize
        pad_size = kk // 2 if kk != config.stride else 0
    ...
    def _get_ref_points(self, H_key, W_key, B, dtype, device):
        ref_y, ref_x = torch.meshgrid(
            torch.linspace(-1.0, 1.0, H_key, dtype=dtype, device=device),
            torch.linspace(-1.0, 1.0, W_key, dtype=dtype, device=device),
            indexing="ij",
        )
    ...
    def forward(self, x):
        # Input format B, H, W, C -> permute to B, C, H, W
        ...
        if self.offset_range_factor >= 0 and not self.no_off:
            offset_range = torch.tensor(
                [1.0 / max(Hk - 1.0, 1e-6), 1.0 / max(Wk - 1.0, 1e-6)], device=device
            ).reshape(1, 2, 1, 1)
