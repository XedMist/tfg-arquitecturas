...
@dataclass
class GlobalAttentionMixerConfig(BaseMixerConfig):
    num_heads: int = 32
    attn_drop: float = 0.0
    proj_drop: float = 0.0


@register_mixer(GlobalAttentionMixerConfig)
class GlobalAttentionMixer(nn.Module):
    def __init__(self, config: GlobalAttentionMixerConfig):
        super().__init__()
